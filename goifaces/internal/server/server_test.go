...
// ---------------------------------------------------------------------------
// HTTP handler tests for ServeInteractiveNoData / handleLoad
// ---------------------------------------------------------------------------

// newTestMux builds the same mux that ServeInteractiveNoData creates, but
// without starting a real server. Tests use httptest.NewServer on the returned
// mux instead.
func newTestMux(t *testing.T) (*http.ServeMux, *serverState) {
	t.Helper()

	landingTmpl, err := template.New("landing").Parse(landingHTMLTemplate)
	require.NoError(t, err, "parsing landing template")

	interactiveTmpl, err := template.New("interactive").Parse(interactiveHTMLTemplate)
	require.NoError(t, err, "parsing interactive template")
	...
	mux.HandleFunc("/", func(w http.ResponseWriter, r *http.Request) {
		...
		td, tdErr := prepareTemplateData(*data)
		if tdErr != nil {
			http.Error(w, "Internal Server Error", http.StatusInternalServerError)
			return
		}
...
func TestLoadEndpointRejectsEmptyPath(t *testing.T) {
	mux, _ := newTestMux(t)
	ts := httptest.NewServer(mux)
	defer ts.Close()

	// Send a POST with an empty path field.
	resp, err := http.Post(ts.URL+"/api/load", "application/json",
...
func TestLoadEndpointRejectsGitHubURLs(t *testing.T) {
	mux, _ := newTestMux(t)
	ts := httptest.NewServer(mux)
	defer ts.Close()
	...
	for _, u := range urls {
		...
		assert.Equal(t, http.StatusBadRequest, resp.StatusCode, "URL: %s", u)

		var body map[string]string
		require.NoError(t, json.NewDecoder(resp.Body).Decode(&body))
		assert.Equal(t, "use -path flag for GitHub URLs", body["error"], "URL: %s", u)
		resp.Body.Close()
	}
}

func TestLoadEndpointRejectsNonPOST(t *testing.T) {
	mux, _ := newTestMux(t)
	ts := httptest.NewServer(mux)
	defer ts.Close()
	...
	for _, method := range methods {
		req, err := http.NewRequest(method, ts.URL+"/api/load", nil)
		require.NoError(t, err)
...
func TestLoadEndpointRejectsMalformedJSON(t *testing.T) {
	mux, _ := newTestMux(t)
	ts := httptest.NewServer(mux)
	defer ts.Close()
	...
	for _, payload := range payloads {
		...
		assert.Equal(t, http.StatusBadRequest, resp.StatusCode,
			"payload %q should be rejected", payload)

		var body map[string]string
		require.NoError(t, json.NewDecoder(resp.Body).Decode(&body))
		assert.Equal(t, "invalid JSON", body["error"],
			"payload %q should return invalid JSON error", payload)
		resp.Body.Close()
	}
}

func TestLandingPageRenderedWhenNoData(t *testing.T) {
	mux, _ := newTestMux(t)
	ts := httptest.NewServer(mux)
	defer ts.Close()
	...
	// The landing page must contain key elements from the landing template.
	assert.Contains(t, body, `id="repo-path"`,
		"landing page response should contain the repo-path input")
	assert.Contains(t, body, `id="analyze-btn"`,
		"landing page response should contain the analyze button")
	assert.Contains(t, body, `id="status"`,
		"landing page response should contain the status element")
	assert.Contains(t, body, "goifaces",
		"landing page response should contain the product name")
}
